...
string segmentb(const string &s){
	if(s == "local") return "LOCAL";
	if(s == "argument") return "ARG";
	if(s == "this") return "THIS";
	if(s == "that") return "THAT";
	return "";
}

int label = 0;

string binary_operation(const string &cmd){
	ostringstream out;
	out << "@SP\nAM=M-1\nD=M\nA=A-1\n";
...
string unary_operator(const string &cmd){
	ostringstream out;
	out << "@SP\nA=M-1\n";
...
string jump_operator(const string &cmd){
	ostringstream out;
...
string push(const string &segment,int index,const string &filename){
	ostringstream out;
    ...
    else out<<"@"<<segmentb(segment)<<"\nD=M\n@"<<index<<"\nA=A+D\nD=M\n";
    out<<"@SP\nA=M\nM=D\n@SP\nM=M+1\n";
	return out.str();
}

string pop(const string &segment,int index,const string &filename){
	ostringstream out;
	if(segment=="static") out<<"@SP\nAM=M-1\nD=M\n@"<<filename<<"."<<index<<"\nM=D\n";
    else if(segment=="temp") out<<"@SP\nAM=M-1\nD=M\n@"<<(5+index)<<"\nM=D\n";
    else if(segment=="pointer") out<<"@SP\nAM=M-1\nD=M\n@"<<(index==0 ? "THIS" : "THAT")<<"\nM=D\n";
    else {
        out<<"@"<<segmentb(segment)<<"\nD=M\n@"<<index<<"\nD=A+D\n@R13\nM=D\n";
        out<<"@SP\nAM=M-1\nD=M\n@R13\nA=M\nM=D\n";
	}
	return out.str();
}

string arithmetic(string cmd){
	label = 0;
	ostringstream out;
	if(cmd == "add"||cmd=="sub"||cmd=="and"||cmd=="or") out << binary_operation(cmd);
	if(cmd =="not" || cmd =="neg") out << unary_operator(cmd);
	if(cmd=="eq"||cmd=="gt"||cmd=="lt") out << jump_operator(cmd);
	return out.str();
}

string pushpop(const string &cmd,const string &segment,int index,const string &filename){
	ostringstream out;
	if(cmd == "push") out << push(segment,index,filename);
	if(cmd == "pop") out << pop(segment,index,filename);
	return out.str();
}
...
void translateFile(const fs::path& filepath, ostream& outStream, const string& filename, string& currentFunction){
    ...
    while(getline(fin, rawLine)){
        ...
        if(cmd=="push"||cmd=="pop"){
            ...
            outStream<<pushpop(cmd,arg1,idx,filename);
        }
        else if(cmd=="label"){
