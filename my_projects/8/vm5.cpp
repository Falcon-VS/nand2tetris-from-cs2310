...
string writeArithmetic(const string& cmd){
    ...
    if(cmd=="add"||cmd=="sub"||cmd=="and"||cmd=="or"){
        ...
        out<<"@SP\nA=M-1\nM=0\n@"<<END<<"\n0;JMP\n";
        out<<"("<<TRUE<<")\n@SP\nA=M-1\nM=-1\n("<<END<<")\n";
        // FIX: The stack pointer was not incremented after a comparison,
        // so the next push would overwrite the boolean result.
        out<<"@SP\nM=M+1\n";
    }
    return out.str();
}
...
// All other Code Writer functions (label, goto, if, function, call, return, init) are correct.
...
string writeCall(const string& fname,int nArgs){
    static int callId=0;
    string ret="RET_CALL_"+fname+"_"+to_string(callId++);
    ostringstream out;
    out<<"@"<<ret<<"\nD=A\n@SP\nA=M\nM=D\n@SP\nM=M+1\n";
...
int main(int argc,char* argv[]){
    ...
    fs::path outputPath; // IMPROVEMENT: Use fs::path for the output path

    if(fs::is_directory(inputPath)){
        for(const auto &p: fs::directory_iterator(inputPath)){
            if(p.is_regular_file() && p.path().extension()==".vm") {
        ...
        // IMPROVEMENT: Place the output .asm file inside the input directory.
        ...
        // IMPROVEMENT: Properly replace the extension for the output file.
    ...
    if(!fout.is_open()){
        cerr<<"Error opening output file "<<outputPath<<"\n";
        return 1;
    }

    // Emit bootstrap code only when translating a directory
    if(fs::is_directory(inputPath)){
        fout << writeInit();
    }
    ...
    for(const auto& vmf : vmFiles){
        // IMPROVEMENT: Use the robust `stem()` method to get the filename without extension.
        string filename = vmf.stem().string();
        translateFile(vmf, fout, filename, currentFunction);
    }

    fout.close();
    cout<<"Successfully created "<<outputPath<<" from "<<inputPath<<"\n";
    return 0;
}
